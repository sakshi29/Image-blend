    ...
    def pers(self,x12): 
        ...
        if len(good)>MIN_MATCH_COUNT:
            ...
            for i in range (0,x12):
                ##gradientX=alpha*(i/y2)
                
                for j in range (0,x2-1):
                     ##gradientY=alpha*(j/x2)
                     ##gradientdecrease=max([gradientX,gradientY])
                     ##gradientdecrease=gradientX
                     ##tarblend=gradientdecrease
                     ##srcblend=1.0-tarblend
                     ##print gradientdecrease
                     
                    
                     out[j,i][0]=out[j,i][0]*alpha+self.img2[j,i][0]*(1-alpha)
                     out[j,i][1]=out[j,i][1]*alpha+self.img2[j,i][1]*(1-alpha)
                     out[j,i][2]=out[j,i][2]*alpha+self.img2[j,i][2]*(1-alpha)
            ...
            for i in range (x12+1,y2-1):
               ## alpha=alpha-0.10 
                 ...
                 for j in range (0,x2-1):
                     ...
                     ##gradientdecrease=max([gradientX,gradientY])
                     gradientdecrease=gradientX
                     srcblend=gradientdecrease
                     tarblend=1.0-srcblend
                     print srcblend
                     ##srcblend=alpha-gradientdecrease
                     ##tarblend=1.0-srcblend
                      
                    
                     out[j,i][0]=self.img2[j,i][0]*srcblend+out[j,i][0]*tarblend
                     out[j,i][1]=self.img2[j,i][1]*srcblend+out[j,i][1]*tarblend
                     out[j,i][2]=self.img2[j,i][2]*srcblend+out[j,i][2]*tarblend
            ...
            for j in range (0,x2-1):
                  ...
                  out[j,i][0]=out[j,i][0]*alpha+self.img2[j,i][0]*(1-alpha)
                  out[j,i][1]=out[j,i][0]*alpha+self.img2[j,i][1]*(1-alpha)
                  out[j,i][2]=out[j,i][0]*alpha+self.img2[j,i][2]*(1-alpha)
            """
